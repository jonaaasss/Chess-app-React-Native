...
export const PIECE_ANIM_DURATION_MS = 650;
...
function squareIndexOf(square: string) {
  return { file: 'abcdefgh'.indexOf(square[0]), rank: '87654321'.indexOf(square[1]) };
}
...
export function usePieceAnimation(pieces: Pieces, cell: number) {
  ...
  useEffect(() => {
    ...
    const newGhosts: Ghost[] = moves.map((m, i) => {
      const from = squareIndexOf(m.from);
      return {
    ...
    Animated.parallel(
      newGhosts.map((g, i) => {
        const to = squareIndexOf(moves[i].to);
